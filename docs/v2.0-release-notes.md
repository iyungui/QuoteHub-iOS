...
앱 실행 시 여러 데이터를 TaskGroup으로 동시에 불러오는데, 이때 Access Token이 만료된 상태라면 **여러 요청이 거의 동시에 401을 받아 각자 토큰 갱신을 시도**하는 문제가 있었습니다. 동일한 갱신 요청이 중복 발생했고, 서버의 Refresh Token 정책에 따라서는 일부 갱신이 실패해 의도치 않은 로그아웃으로 이어질 수 있는 구조였습니다.

원인은 네트워크 계층(APIClient)이 갱신 동작을 직렬화하지 않는다는 점이었습니다. 동시에 6개의 요청이 갱신을 트리거하면 실제 갱신이 6번 실행되는 것을 로그로 확인했습니다.

이를 **single-flight** 패턴으로 해결했습니다. 진행 중인 갱신 작업(Task)을 actor에 보관하고, 첫 요청만 실제 갱신을 트리거하며 나머지 요청은 그 Task의 결과를 기다려 공유하도록 했습니다. 그 결과 동시에 여러 요청이 401을 받더라도 **실제 토큰 갱신은 한 번만** 실행됩니다 (6개 동시 요청 → 갱신 1회 확인).

이 과정에서 actor의 **재진입(reentrancy)** 때문에 단순 플래그로는 막을 수 없어 "진행 중인 Task 자체를 공유"해야 한다는 점, 그리고 갱신 엔드포인트가 인증을 요구하면 자기 자신을 기다리는 교착에 빠질 수 있다는 점까지 이해하게 되었습니다.
