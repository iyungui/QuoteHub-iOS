...
초기 1.0 버전 구현에서는 단순히 각 로드 작업을 순차적으로 await 하여 실행했지만, 대부분의 작업이 서로 의존성이 없는 독립적인 네트워크 요청이라는 점에 주목했습니다.
**기존의 순차 실행 방식에서는 각 요청의 결과를 대기하는 시간이 누적되기 때문에 초기로딩 속도가 불필요하게 증가할 수 있었습니다.** 
